...
function get_page_data()
	{
		...
		if($data_type == 'product')
		...
		else if($data_type == 'category')
			{
				...
				$return['meta']['title'] = "".$return['product']['alco_19']." ".$return['product']['alco_29']." ".$return['product']['alco_40']." в ".$return['product']['alco_141']." по низким ценам";
				$return['meta']['description'] = "".$return['product']['alco_26']." ".$return['product']['alco_19']." ".$return['product']['alco_29']." у ".$return['product']['alco_147']." на портале 🌐 Mopito.ru. Лучшие ".$return['product']['alco_146']." ".$return['product']['alco_37']." в ".$return['product']['alco_141']." и ".$return['product']['alco_142'].". ☎️ Звоните ".$return['product']['alco_149']." прямо с сайта Мопито ру - все ".$return['product']['alco_146']." ".$return['product']['alco_37']." ".$return['product']['alco_152']."";
...
header('Last-Modified: '. $LastModified);
		...
		else if($data_type == 'podcategory')
			{
				$result = route_query($request_uri);
				$return['product'] = mysqli_fetch_array($result);
				$return['meta']['title'] = "".$return['product']['alco_20']." ".$return['product']['alco_29']." ".$return['product']['alco_40']." в ".$return['product']['alco_141']." по низким ценам";
				$return['meta']['description'] = "".$return['product']['alco_26']." ".$return['product']['alco_53']." ".$return['product']['alco_29']." у ".$return['product']['alco_147']." ".$return['product']['alco_37']." на сайте 🌐 Mopito.ru. Лучшие ".$return['product']['alco_146']." ".$return['product']['alco_24']." в ".$return['product']['alco_141']." и ".$return['product']['alco_142'].". ☎️ Звоните ".$return['product']['alco_149']." прямо с портала Мопито ру - все ".$return['product']['alco_146']." ".$return['product']['alco_24']." ".$return['product']['alco_152']."";
			               // можно еще так реализовать эту функцию "Мы нашли для Вас $result->num_rows лучших поставщиков ".$return['product']['alco_24']." ...
			    $return['meta']['canonical'] = "https://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
				$return['page']['count_products'] = $result->num_rows;
				$return['page']['data_type'] = $data_type;
...
function get_block_description($row)
{
	$return = str_replace('??','И',mb_convert_encoding($row['alco_21'],'UTF-8')).'
				<span class="text600">'.$row['alco_151'].'</span>';
	return $return;
}
