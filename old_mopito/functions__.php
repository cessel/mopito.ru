...
function get_page_data()
	{
		...
		if($data_type == 'product')
		...
		else if($data_type == 'category')
			{
				...
				$return['meta']['title'] = "".$return['product']['alco_26']." ".$return['product']['alco_19']." ".$return['product']['alco_29']." в ".$return['product']['alco_141']." по низким ценам - mopito.ru";
				$return['meta']['description'] = "Мы нашли для Вас $result->num_rows лучших поставщиков ".$return['product']['alco_37']." ".$return['product']['alco_29']." в ".$return['product']['alco_141']." и ".$return['product']['alco_142'].". 🌐 Мопито ру - все поставщики в магазины и бары";
				$return['meta']['canonical'] = $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
				...
				$return['page']['title'] = mb_ucfirst($return['product']['alco_19']).' оптом';
		...
		else if($data_type == 'podcategory')
			{
				$result = route_query($request_uri);
				$return['product'] = mysqli_fetch_array($result);
				$return['meta']['title'] = "".$return['product']['alco_26']." ".$return['product']['alco_20']." ".$return['product']['alco_29']." в ".$return['product']['alco_141']." по низким ценам - mopito.ru";
				$return['meta']['description'] = "Мы нашли для Вас $result->num_rows лучших поставщиков ".$return['product']['alco_20']." ".$return['product']['alco_29']." в ".$return['product']['alco_141']." и ".$return['product']['alco_142'].". 🌐 Мопито ру - все поставщики в магазины и бары";				$return['meta']['canonical'] = $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
				$return['page']['count_products'] = $result->num_rows;
				$return['page']['data_type'] = $data_type;
				$return['page']['top_menu_title'] = mb_ucfirst($return['product']['alco_20']);
				$return['page']['title'] = mb_ucfirst($return['product']['alco_20']).' оптом';
						 // $return['page']['top_menu_title'] = "Смотри function.php строка 276"; // можно задать любые парамерты из базы данных например $return['product']['alco_19'], или сочетание параметров...
			}
...
function get_block_header($row)
{
	$return =
		'<div class="blocktitle '.$row['alco_127'].'">
				<div class="blockh3left moly-1"><a href="/'.$row['alco_12'].'/'.$row['alco_18'].'/'.$row['alco_22'].'/'.$row['alco_70'].'/" title="'.$row['alco_15'].'">'.$row['alco_30'].' '.$row['alco_38'].' '.$row['alco_32'].', '.$row['alco_11'].', '.$row['alco_10'].', <span class="grey1">'.$row['alco_111'].'</span>
				</a></div>
...
{
	$return = '<p class="textbig textpall text-ale1">'.$row['alco_31'].'</p>
...
function get_block_promo_info($row)
{
	...
	$return = '
			<div class="'.$row['alco_130'].' infohot">
				<div class="statushot" data-title="Горячие предложения Партнера - доступно от 2-х приоритетов">
                    <span class="promohotimg" alt="Акции доступны от 2-х приоритетов">
					   '.$row['alco_138'].'
				    </span>
				</div>
