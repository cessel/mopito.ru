...
while($row_clicks = mysqli_fetch_array($result_clicks)) 

	{
		...
		foreach($data[$row_clicks['url']] as $d)
			...
			{
				$prefix = ($row_clicks['clicks_'.$k] !== 0) ? '=' :'';
				$line[$j][] = $prefix.'"'.$d.'/'.$row_clicks['clicks_'.$k++].'"';
...
foreach($header as $h)

	{
		...
		$new_header[] = mb_convert_encoding($h,'CP1251','UTF-8');
...
echo "<a href='/import/".$filename."'>Скачать файл</a>";
