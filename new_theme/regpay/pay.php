...
<style>
    ...
    #dsl1 label, #dsl2 label, #dsl3 label, #dsl4 label{ display: inline-block; padding: 8px 7px; border: solid #000 1px; border-radius: 30px 0 0 30px;}
    ...
    #month{ padding: 5px 0; display: block; margin: auto;}
...
<div id="form">
  <form id="form1">
    ...
    <div id="dsl1" style="display:none;">
    	<label class="dsl123block">Выбор услуги: </label>
      <select class="dsl123block-sel" id="sl1" onChange="sel(this)">
    	...
    	<label class="dsl123block">Кол-во мес: </label>
      <select class="dsl123block-sel" data-price="7000" id="sl2" onChange="sel2(this, 'dsl4'); calc(false);">
      	<option>Выбрать</option>
      	<option>1</option>
      	<option>2</option>
      	<option>3</option>
    	...
    	<label class="dsl123block">Кол-во приоритетов: </label>
      <select class="dsl123block-sel" data-price="35" id="sl3" onChange="sel2(this, 'dsl4'); calc(false);">
      	<option>Выбрать</option>
      	<option>1</option>
      	<option>2</option>
      	<option>3</option>
    	...
    	<label class="dsl123block">Способ оплаты: </label>
      <select class="dsl123block-sel" id="sl4" onChange="calc(true); regulations2(this);">
      	<option>Выбрать</option>
      	<option data-bankAccount="41001726860097">Яндекс-деньги</option>
      	<option data-bankAccount="6390-0240-9019-5203-58">Сберкарта (Анна Андреевна М.)</option>
      </select>
    </div>
    ...
    <div id="dsl5">
      <b id="Click">Выставить счет</b>
		  ...
		  <div class="nav-tabs nav-stacked nav-coupon-category ingreytext">
                <span>*Укажите действующий ID вашего объявления. Если объявление уже в архиве обратитесь в тех.поддержку </span>
            </div>
        </div>
      </div>
	  <div class="col-xs-12">
            <div>
                <h3>Оплата объявлений</h3>
                <p>	Оплату можно производить при наличии действующего объявления. Для этого нужно скопировать ID объявления (он указан в заголовке Вашего объявления и имеет вид ID9343-msk 25.10.2020 P-05) и вставить в ячейку. Далее выбрать "Аренду" для продление действия объявления, или "Приоритеты" если вы хотите поднять объявление на странице, выделить цветом или увеличить кол-во звездочек статуса. После отправки формы для оплаты,  данные проходят модерацию, а вам на почту (которую указывали при регистрации фирмы) прийдет форма для дальнейшей оплаты. Плата за услуги начинают действовать в 00:00 по московскому времени в соответствии с общим количеством Приоритетов или Аренды. Если оплата не будет произведена, объявление переходит в статус "Архивное", и ее позиции понизяться в ранжировании. Чтобы ваши объявления оставались на занятых позициях, вовремя оплачивайте услуги. В любом случае, если появяться вопросы обращайтесь в техническую поддержку портала Мопито.ру.</p>
