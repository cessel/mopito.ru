...
function Submit(){
	if($("#g-recaptcha-response").val() != ''){
		...
		document.getElementById('form').innerHTML = "<div style=\"width:280px; margin:100px auto; background:#39F; text-align:center; font-size:24px; padding: 10px;\">Ваше объявление отправлено на модерацию</div>";
	}
}
...
function inpId(bull, $this, id){
	if(bull && $this.id == 'id'){
		...
		maxMarker = Date.parse('12 31 2019'); //Максимальная метка
		...
		if(insertMarker<thisMarker){
			document.getElementById('error').innerHTML = '<div>указанный ID просрочен. Обратитесь к администратору</div>';
			...
			document.getElementById('error').innerHTML = '<div>указанный ID неверен. Обратитесь к администратор</div>';
			document.getElementById('dsl1').getElementsByTagName('option')[0].selected=true;
			document.getElementById('dsl1').setAttribute('style', 'display:none;');
			document.getElementById('dsl2').getElementsByTagName('option')[0].selected=true;
			document.getElementById('dsl2').setAttribute('style', 'display:none;');
...
function Submit2(){
	...
	if($("#g-recaptcha-response").val() != '' && further){
		...
		document.getElementById('form').innerHTML = "<div style=\"width:280px; margin:100px auto; background:#39F; text-align:center; font-size:24px; padding: 10px;\">Ваш запрос отправлен менеджеру, который с Вами свяжется в ближайшее время. Спасибо</div>";
	}
}